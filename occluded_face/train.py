...
def parse_args():
    ...
    parser.add_argument('--use-class_weight', action='store_true')
...
def main():
    ...
    train_set, valid_set, test_set = split_dataset(args.img_dir, args.split_ratio)
    ...
    checkpoint_dir = os.path.join(args.output_dir, args.task, 'weight' if args.use_class_weight else 'none')

    model = MobileNetV2(num_classes=train_set.num_classes).to(device)

    weight = train_set.class_weights.to(device) if args.use_class_weight else None
    ...
    scheduler = ReduceLROnPlateau(optimizer, 'min', patience=args.epochs // 5, verbose=True)
    ...
    if args.metric:
        ...
        with open('results.txt', 'a+') as f:
            f.write(f'{args.task}-{args.use_class_weight} Epeochs: {args.epochs} \n')
